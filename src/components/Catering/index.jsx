...
import CardCustom from "../Helpers/Cards/CardCustom";
import DataIteration from "../Helpers/DataIteration";
import PageTitle from "../Helpers/PageTitle";
import ServeLangItem from "../Helpers/ServeLangItem";
import ViewMoreTitle from "../Helpers/ViewMoreTitle";
import Star from "../Helpers/icons/Star";
function Catering({ sellersData, className,
  sectionTitle,
  seeMoreUrl, }) {
  ...
  // console.log(selldata.map((product) => {
  //   return product;
  // }), 'tr');
  selldata.forEach((seller) => {
    // console.log(`Seller: ${seller.shop_name}`);
  ...
  // Create an array to store the extracted products
  const extractedProducts = [];

  // Iterate through the selldata array to extract products
  // selldata.forEach((seller) => {
  //   seller.products.forEach((product) => {
  ...
  const rs = extractedProducts.slice(0, 4).map((item) => {
    return {
      id: item.id,
      title: item.name,
      slug: item.slug,
                    ...
                    <ViewMoreTitle categoryTitle={sectionTitle} seeMoreUrl={seeMoreUrl}>
                      <div className="products-section w-full">
                        <div className="grid xl:grid-cols-4 lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 xl:gap-[30px] gap-5">
                          <DataIteration datas={seller.products} startLength={0} endLength={seller.products.length}>
                            {({ datas }) => (
